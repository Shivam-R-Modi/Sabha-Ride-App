...
describe('tokens that must NOT be used for text are still unsafe', () => {
    it.each(['--accent', '--gold'])('%s remains below AA, as documented', (role) => {
        // If one of these ever clears 4.5:1 someone has quietly changed the
        // brand colour, and the ramps that exist to work around it are stale.
        expect(contrast(THEMES.light[role], THEMES.light['--canvas'])).toBeLessThan(AA_NORMAL);
    });
});
