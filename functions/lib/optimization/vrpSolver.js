...
function solveCVRP(students, drivers, rideType) {
    ...
    let routes = clarkeWrightSavings(students.length, distanceMatrix, vehicleCapacities, drivers);
    ...
    for (let i = 0; i < n; i++) {
        ...
        for (let j = 0; j < n; j++) {
            if (i === j) {
                ...
                matrix[i][j] = (0, distance_1.haversineDistance)(locations[i], locations[j]);
            }
        }
    }
    return matrix;
 ...
 */
function clarkeWrightSavings(numStudents, distanceMatrix, vehicleCapacities, drivers) {
    var _a, _b;
    ...
    for (let v = 0; v < numVehicles; v++) {
        routes.push({
            driverId: ((_a = drivers[v]) === null || _a === void 0 ? void 0 : _a.id) || `vehicle_${v}`,
            driverName: ((_b = drivers[v]) === null || _b === void 0 ? void 0 : _b.name) || `Vehicle ${v}`,
    ...
    const studentRoute = new Map(); // student index -> route index
 ...
 */
function optimizeRouteOrder(studentIdsInRoute, distanceMatrix) {
    if (studentIdsInRoute.length <= 1)
        return studentIdsInRoute;
 ...
 */
function improveRoutesWithLocalSearch(routes, distanceMatrix, students) {
    let improved = true;
    let iterations = 0;
 ...
 */
function twoOpt(studentIdsInRoute, distanceMatrix) {
    if (studentIdsInRoute.length < 3)
        return studentIdsInRoute;
 ...
 */
function calculateRouteDistance(studentIdsInRoute, distanceMatrix) {
    if (studentIdsInRoute.length === 0)
        return 0;
    let distance = 0;
 ...
 */
function tryRelocate(fromRoute, toRoute, distanceMatrix) {
    if (fromRoute.studentIds.length === 0)
        return { improved: false, fromRoute, toRoute };
    if (toRoute.totalDemand >= toRoute.capacity)
        return { improved: false, fromRoute, toRoute };
...
//# sourceMappingURL=vrpSolver.js.map