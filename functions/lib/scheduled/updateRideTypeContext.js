...
exports.attendanceHeaderChanged = attendanceHeaderChanged;
 ...
 */
function attendanceHeaderChanged(current, event) {
    var _a, _b;
    if (!current)
        return true;
    if (current.eventId !== event.eventId)
        return true;
    for (const field of ['startsAt', 'endsAt', 'attendanceLocksAt']) {
    ...
    const before = ((_a = current.venue) !== null && _a !== void 0 ? _a : null);
    const after = ((_b = event.venue) !== null && _b !== void 0 ? _b : null);
    if (!before !== !after)
        return true;
    if (before && after && (before.lat !== after.lat || before.lng !== after.lng))
        return true;
    return false;
}
/**
...
exports.updateRideTypeContext = functions.pubsub
    ...
    try {
        ...
        const scheduled = await (0, events_1.findCurrentEvent)(db, now, timeZone, rule);
