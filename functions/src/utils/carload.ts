...
/** What `orderForCarload` needs to know about a waiting request. */
export interface Waiting extends LightPoint {
    /** Half of a group already split across cars. */
    isRemainder?: boolean;
    /** ISO timestamp the request was made. Absent means unknown, never "now". */
    createdAt?: string;
}
...
function waitedMs(r: Waiting, now: number): number {
 ...
 */
export function chooseSeed<T extends Waiting>(
    pool: T[],
    venue: { lat: number; lng: number },
    now: number = Date.now(),
    escalationMs: number = WAIT_ESCALATION_MS,
): T | null {
    ...
    return tier.reduce((far, r) =>
        milesBetween(r.lat, r.lng, venue.lat, venue.lng)
 ...
 */
export function orderForCarload<T extends Waiting>(
    pool: T[],
    venue: { lat: number; lng: number },
    now: number = Date.now(),
    escalationMs: number = WAIT_ESCALATION_MS,
