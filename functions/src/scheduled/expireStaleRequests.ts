 ...
 * Exported for tests.
 */
export async function clearEndOfEveningStatuses(
    ...
    if (lingering.empty) return 0;

    const travelling = new Set(
        live.docs.map(d => d.data()?.studentId).filter((id): id is string => typeof id === 'string'),
    );
    ...
    for (const doc of lingering.docs) {
        if (travelling.has(doc.id)) continue;
