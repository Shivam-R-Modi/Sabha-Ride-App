...
| D2 | ~~Age bands only~~ | **PARTLY REVERSED 2026-08-25.** Still true of the sabha service: no `ageBand` field was ever built and no DOB is stored there. **Airport Seva stores an exact `dateOfBirth`** on `airportProfiles/{uid}` and snapshots it onto each trip, by owner decision — a Sarthi meeting a stranger at an arrivals gate needs to confirm they have the right person. Contained rather than open: `airportProfiles` is readable only by its owner and by an airport coordinator (`isAirportCoordinator()`), so the durable record is not on the board every Sarthi can browse. | Excess sensitive data — accepted for the airport service, see D2 in the register |
