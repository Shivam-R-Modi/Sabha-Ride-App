...
| D3 | Dependent travels with guardian | Request time: a dependent cannot be added unless the guardian is also a passenger. Assignment: manifest validated again server-side | Unaccompanied child in a volunteer's car |
...
| D5 | Max 3 guests | Request form caps at 3; Cloud Function re-validates; additionally bounded by belted seats | Group exceeds vehicle capacity |
| D6 | Driver vetting gate | Assignment refuses any driver whose `vetting.status != 'approved'` or whose `vetting.expiresAt` has passed | Unvetted adult transporting minors |
...
| D9 | Tenancy isolation | Custom claims + rules scoped by `locationId` | One location reads the national dataset |
...
// users/{uid}
{
  ...
  "memberships": { "<locationId>": "student" },
  ...
  "vetting": {                              // drivers only (D6)
    "status": "approved",                   // none|pending|approved|expired|rejected
    "checks": [ { "type": "mvr", "completedAt": "…", "expiresAt": "…" } ],
    "reviewedBy": "<uid>"
    // outcome and dates ONLY — never the underlying report
  }
}
...
match /users/{userId} {
  allow read: if isOwner(userId)
              || isGuardianOf(userId)
              || sharesManagedLocation(userId)
              || isSuperManager();
}
```

Authorisation should move to **custom claims** (`token.mgr` = locations
managed, `token.sm` = super-manager) rather than `getUserData()`, which costs a
...
1. Driver `vetting.status == 'approved'` and not expired → else reject (D6)
2. Ride `locationId` matches the driver's active location → else reject (D9)
3. `seatCount <= availableBeltedSeats` → else do not place (D5)
4. Every `under13` dependent has its guardian in the same manifest (D3)
5. If `preventOneToOneWithMinor` — reject a lone `13-17` in an empty vehicle (D4)
6. Group is never split across vehicles
...
| Vetting expiry sweep | Daily | Flips drivers to `expired`, notifies them and their manager (D6) |
...
`location.created` · `location.archived` · `vetting.statusChanged` ·
`schedule.changed` · `incident.reported` · `legalHold.applied` ·
...
| 4 — Server dispatch | D4 gate, D6 gate, group-integrity constraint |
| 5 — Events | Schedule audit events |
| 6 — Super-manager | Role governance, audit, impersonation logging |
...
architecture. The clauses needing review (D1, D6, D7) affect *values and
