...
| D2 | **Age is stored as a band** (`under13` / `13-17` / `adult`), never a date of birth — **except in Airport Seva, which stores an exact `dateOfBirth`.** | The original rationale stands for the ride service and no `ageBand` field was ever built there either. It does not stand for airport pickups: a Sarthi meets somebody they have never seen, at a barrier, often with no working phone on either side, and a date of birth is what settles that they have the right person. Owner decision on 2026-08-25, taken with the note that this is a reversal on record. Mitigation, engineering's addition: the field lives on `airportProfiles`, which is readable only by its owner and by an airport coordinator — never by an ordinary Sarthi. The per-trip copy a Sarthi does see is scoped to the one traveller in front of them and disappears from the DOM when the card is collapsed. | **Reversed for Airport Seva by owner — 2026-08-25** |
