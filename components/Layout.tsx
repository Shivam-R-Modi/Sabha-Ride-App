        ...
        {navItems.map((item) => {
          ...
          return (
            <button
              ...
              className={`w-full flex items-center gap-4 p-3 rounded-2xl transition-all group relative btn-feedback ${
                isActive 
...
const BottomNav: React.FC<{ role: UserRole }> = ({ role }) => {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 bg-white shadow-[0_-8px_30px_rgba(0,0,0,0.06)] border-t border-orange-50 z-50 pb-safe lg:hidden">
        ...
        {navItems.map((item) => {
          ...
          return (
            <button
              key={item.id}
              onClick={() => setCurrentTab(item.id as TabView)}
              className={`relative flex flex-col items-center justify-center h-full w-full transition-all btn-feedback ${
