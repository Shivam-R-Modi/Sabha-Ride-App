...
export const PickupForm: React.FC<PickupFormProps> = ({ user, onClose, onSubmit }) => {
  ...
  return (
    <div className="bg-white rounded-3xl shadow-xl overflow-hidden border border-orange-100/50 rangoli-border mx-4 mt-8">
      ...
      <div className="p-8 space-y-6">
        ...
        <div className="pt-4">
           <button 
                ...
                disabled={isLoading}
                className="w-full bg-gradient-to-r from-saffron to-saffron-dark text-white font-bold py-4 rounded-2xl shadow-xl shadow-orange-200 active:scale-95 btn-feedback transition-all flex items-center justify-center gap-2"
            >
                {isLoading ? (
                    <LotusLoader size={24} />
                ) : (
